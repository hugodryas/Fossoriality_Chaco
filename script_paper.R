...
# Drivers of verticalit stratification # Analizamos las variables que influyen en la estratificación vertical
...
# Load snake trait data and prepare verticality-related variables: #cargamos nuestras datos y las preparamos
trait_data<-fread("trait_data.csv", stringsAsFactors=TRUE, encoding="UTF-8")

# Make 'Habitat' an ordinal variable: #en estos y los siguientes pasos vamos a ir convirtiendo las variables para su analisis. En el MS se explica bien
...
assemblage_data<-fread("snk_assamble55.csv", stringsAsFactors=TRUE, encoding="UTF-8")
selected_cells<-fread("cells_chaco55.csv", stringsAsFactors=TRUE, encoding="UTF-8")
...
selected_cells<-fread("cells_chaco55.csv", stringsAsFactors=TRUE, encoding="UTF-8")
setwd("")
terr_cover <- readOGR("Chaco_TNC2005.shp")
grid_cells <- readOGR("gridcells_55km.shp")
##not used
terr_cover<-readOGR(dsn='Shapefiles',layer='Chaco_TNC2005')
grid_cells<-readOGR(dsn='Shapefiles',layer='gridcells_110km')
grid_cells<-grid_cells[grid_cells@data$PageNumber %in% as.character(selected_cells$Cell_Id55),]
rm(selected_cells)
...
# STEP 6 - PERFORM THE ANALYSIS OF CORRELATES OF SNAKE VERTICALITY
############################################################################################################################
# STEP 6 - PERFORM THE ANALYSIS OF CORRELATES OF SNAKE VERTICALITY
...
setwd("C:/Users/Hugodryas/Documents/Documentos/Doctorado/Base de Datos/Fosorial/Raster/DataFrame variables/Traits Final")
rm(list=ls())

data_snakes<- read.delim("traits_variables.csv", sep = ";")

names(data_snakes)
head(data_snakes, n=10)
...
View(t(avg_model))
#transpose and export avg model results to make figures
...
rm(list=ls()); gc()
setwd("")
grid_cells <- readOGR("") #the grid youre using for 
...
community <- read.delim("", sep=";", row.names = 1)
...
##You can export the results, if you want to work with GIS programs.
##PSV
comunity<- read.delim("matrix_t.csv", sep=";", row.names = 1)
comunity$X.1<- NULL
